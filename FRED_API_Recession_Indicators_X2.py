...
if any(x is None for x in [yield_spread, nasdaq, jobless_claims, real_gdp, recession]):
    print("Failed to fetch one or more series for first plot. Exiting.")
    exit(1)
...
# Print latest values for each (with their own dates)
print("Latest Values:")
for col in ['Yield_Spread', 'Nasdaq_YoY', 'Jobless_Claims', 'Real_GDP_YoY']:
    ...
    if latest_idx is not None:
        ...
        if col == 'Yield_Spread':
            print(f"10y-2y Yield Spread: {latest_val:.2f}% (as of {latest_date})")
        elif col == 'Nasdaq_YoY':
            print(f"NASDAQ Composite YoY Change: {latest_val:.2f}% (as of {latest_date})")
        elif col == 'Jobless_Claims':
            print(f"Initial Jobless Claims: {latest_val:.0f} thousand (as of {latest_date})")
        elif col == 'Real_GDP_YoY':
            print(f"Real GDP YoY: {latest_val:.2f}% (as of {latest_date})")
...
if any(x is None for x in [m2, gdp, velocity, cpi, debt, recession]):
    print("Failed to fetch one or more series for second plot. Exiting.")
    exit(1)
...
# Print latest estimates for each
print("\nLatest Estimates:")
for col in ['Real_Inflation', 'CPI_Inflation', 'Debt_YoY']:
    ...
    if latest_idx is not None:
        ...
        if col == 'Real_Inflation':
            print(f"M2-Based Inflation: {latest_val:.2f}% (as of {latest_date})")
        elif col == 'CPI_Inflation':
            print(f"CPI Inflation: {latest_val:.2f}% (as of {latest_date})")
        elif col == 'Debt_YoY':
            print(f"US Debt YoY % Change: {latest_val:.2f}% (as of {latest_date})")
